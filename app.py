...
def model_report():
    ...
    **Taste**
    - Accuracy: {taste_acc:.4f}
    - F1-score (macro): {taste_f1:.4f}
    **Solubility**
    - Accuracy: {sol_acc:.4f}
    - F1-score (macro): {sol_f1:.4f}
    **Docking Score**
    - RMSE: {dock_rmse:.5f}
    - R²: {dock_r2:.5f}
    """
...
def predict_single(sequence: str):
    ...
    if not seq:
        return pd.DataFrame([{"Message": "Enter a sequence to get predictions."}]), None
    validate_seq(seq)
    ...
    feats["Sequence"] = seq
    pdb_file = save_pdb(seq)
    return pd.DataFrame([feats]), pdb_file
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("Single Prediction"):
        ...
        pdb_out = gr.File(label="Download PDB", type="binary")
        seq_in.change(fn=predict_single, inputs=seq_in, outputs=[table_out, pdb_out])
